...
def get_scholar_html_via_oxylabs(scholar_id, username, password):
    """Get Google Scholar profile HTML using Oxylabs API"""
    try:
        # Build the Google Scholar URL
        scholar_url = f"https://scholar.google.com/citations?user={scholar_id}&hl=en"
        logger.info(f"Fetching Scholar URL via Oxylabs: {scholar_url}")
        ...
        response = requests.request(
            ...
            json=payload,
            timeout=120  # Oxylabs can take some time
        )
        ...
        # Extract HTML content from Oxylabs response
        if 'results' in json_data and len(json_data['results']) > 0:
            html_content = json_data['results'][0].get('content', '')
            if html_content:
                logger.info("Successfully received HTML content from Oxylabs")
                return html_content
            else:
                logger.error("No content found in Oxylabs response")
                return None
        else:
            logger.error("No results found in Oxylabs response")
            return None
        
    except requests.exceptions.RequestException as e:
        logger.error(f"Error making request to Oxylabs: {e}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error with Oxylabs API: {e}")
        return None

def get_scholar_stats(scholar_id):
    """Get statistics for a Google Scholar profile using Oxylabs API"""
    ...
    # Parse HTML with BeautifulSoup (same logic as original code)
    ...
    try:
        ...
        # Research interests
        interests = []
        interest_elems = soup.select('#gsc_prf_int .gsc_prf_inta')
        ...
        # Store all metrics in citation_stats
        ...
        scholar_stats = {
            ...
            'source': 'oxylabs_api'  # Add source identifier
        }
...
def main():
    """Main function to retrieve Google Scholar stats via Oxylabs API"""
    try:
        # Load environment variables
        env_vars = load_environment_variables()
        scholar_id = env_vars['SCHOLAR_ID']
        
        # Get scholar stats via Oxylabs API
        logger.info(f"Retrieving stats for Scholar ID: {scholar_id} via Oxylabs API")
        ...
        if scholar_stats:
            ...
            print(f"Interests: {', '.join(scholar_stats['profile'].get('interests', []))}")
